...
contract Proposal is IProposal {
    ...
    constructor(
        ...
        require(
            _authorizationAddress != address(0) && _guardianAddress != address(0) && _timelockAddress != address(0),
            AddressZero()
        );
    ...
    function approveProp(uint256 _proposalId, address _signer)
        external
        override
        onlyAuthorizer
    ...
    function cancelProp(uint256 _proposalId, string memory reason) external override proposalExists(_proposalId) {
        ...
        bool isProposerCancellingEarly =
            (msg.sender == proposal.proposer
                && (proposal.status == propStat.PENDING || proposal.status == propStat.APPROVED));
